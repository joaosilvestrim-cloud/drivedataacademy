        ...
        <div className={styles.surface}>
          ...
          {tab === 'explorar' && <>
            ...
            {comp && score && <aside data-tour="universo-detalhe" className={styles.detail} style={{ '--area': color } as CSSProperties} aria-label={`Detalhes de ${comp.name}`}>
              ...
              <div className={styles.scoreRow}><strong>{score.score}<small>/100</small></strong><span>{score.level}</span></div><p className={styles.scoreLabel}>{tr("DOMÍNIO EVIDENCIADO")}</p>
          ...
          {tab === 'conexoes' && <section className={styles.contentView}><p className={styles.eyebrow}>{tr("EXPANDA SUAS POSSIBILIDADES")}</p><h2>{tr("Próximas conexões")}</h2><p>{tr("O que você já sabe pode abrir um novo caminho. Os requisitos acompanham a data da timeline.")}</p><div className={styles.opportunityGrid}>{catalog.unlocks.map(u => { const s = scores[u.target]; const c = catalog.competencies.find(c => c.id === u.target)!; return <article key={u.target} className={styles.opportunity}><span className={s.ready ? styles.ready : styles.waiting}>{s.ready ? <CheckCircle2 size={14} /> : <LockKeyhole size={14} />}{s.ready ? 'Pronto para começar' : 'Construindo a base'}</span><h3>{c.name}</h3><p>{c.description}</p><div className={styles.bar}><span style={{ width: `${s.readiness*100}%` }} /></div>{flatten(u.rule).map(r => <button key={r.competency} onClick={() => select(r.competency)}><span>{catalog.competencies.find(c => c.id === r.competency)?.name}</span><b>{scores[r.competency].score} / {r.minimum} {scores[r.competency].raw >= r.minimum && <Check size={13} />}</b></button>)}<button className={styles.primary} onClick={() => select(c.id)}>{tr("Explorar competência")} <ArrowUpRight size={15} /></button></article>; })}</div></section>}
