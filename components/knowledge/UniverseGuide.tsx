...
export default function UniverseGuide({isDemo}:{isDemo:boolean}) {
  ...
  return <>
    ...
    <section className={styles.guideNext}><h3>{tr("Como desenvolver meu universo?")}</h3><ol>
      ...
      <li><strong>{tr("Coloque o conhecimento em prática.")}</strong> {tr("Exercícios, desafios e revisões validados pela equipe também contribuem.")} {isDemo?'A aba Desafios permite experimentar um exercício fictício.':'Nesta versão, a equipe registra essas evidências; você as consulta na aba Desafios.'}</li>
