...
export default async function ProfilePreview({userId,email}:{userId:string;email?:string|null}) {
  ...
  if(available) summary=developed?`${developed} competências com evidências registradas. Explore seu histórico e descubra os próximos caminhos.`:tr("Seu universo está pronto para receber as primeiras evidências de aprendizagem.");
  return <section className="relative mt-8 overflow-hidden rounded-2xl border border-teal-300/20 bg-gradient-to-br from-teal-900/20 via-slate-900 to-slate-950 p-6">
    ...
    <div className="relative"><Orbit className="mb-4 text-teal-200" size={24} /><p className="text-[10px] uppercase tracking-[.2em] text-teal-200/70">{tr("Meu universo de conhecimento")}</p><h2 className="mt-2 text-xl font-semibold text-white">{tr("Knowledge Universe 4D")}</h2><p className="mt-2 max-w-sm text-sm leading-relaxed text-slate-300">{tr("Seus estudos viram um mapa de competências em 3D. A quarta dimensão é o tempo: acompanhe o que você desenvolveu e descubra o que aprender a seguir.")}</p><p className="mt-2 max-w-sm text-sm leading-relaxed text-slate-400">{summary}</p><Link href="/universo" className="mt-5 inline-flex items-center gap-3 rounded-lg bg-teal-200 px-4 py-2.5 text-xs font-semibold text-slate-950">{tr("Explorar meu universo")}<ArrowUpRight size={15} /></Link></div>
  </section>;
}
