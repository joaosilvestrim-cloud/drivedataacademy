...
export default function AvisoMateriaisLiberados(){
  ...
  return <aside className="fixed bottom-5 left-4 z-[70] w-[calc(100%-2rem)] max-w-sm rounded-2xl border border-brand-green/40 bg-ink-800 p-5 text-white shadow-2xl sm:left-6" aria-label="Materiais liberados"><div className="flex items-start gap-3"><Download className="shrink-0 text-brand-green" size={23}/><div className="min-w-0 flex-1" role="status"><h2 className="font-display text-lg font-semibold">Seus materiais já estão liberados!</h2><p className="mt-1 text-sm text-slate-300">{aviso.itens.length} {aviso.itens.length===1?"arquivo disponível":"arquivos disponíveis"} para download.</p></div><button type="button" aria-label="Fechar aviso de materiais" onClick={fechar} className="rounded-lg p-2 text-slate-300 hover:bg-white/10 focus-visible:outline focus-visible:outline-brand-green"><X size={18}/></button></div><ul className="mt-4 space-y-2">{cursos.slice(0,3).map(i=><li key={i.cursoId}><Link href={i.href} onClick={fechar} className="block rounded-xl bg-brand-green/10 px-3 py-3 text-sm font-medium text-brand-green hover:bg-brand-green/20">Acessar {i.curso} →</Link></li>)}</ul>{cursos.length>3&&<Link href="/conta/cursos" onClick={fechar} className="mt-3 block text-sm text-brand-green underline">Ver todos os meus cursos</Link>}</aside>;
}
