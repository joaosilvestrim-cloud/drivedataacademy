...
function DataTable({table,label}:{table:Table;label:string}) {
  ...
  return <section className={s.tablePanel}><div className={s.tableHead}><strong>{label}</strong><span>{table.rows.length.toLocaleString('pt-BR')} linhas · {table.columns.length} colunas</span></div><div className={s.tableScroll}><table><thead><tr>{table.columns.map((c,i)=><th key={i}>{c}</th>)}</tr></thead><tbody>{table.rows.slice(safePage*30,safePage*30+30).map((row,i)=><tr key={i}>{row.map((v,j)=><td key={j} title={v===null?'NULL':String(v)}>{v===null?<em>NULL</em>:String(v)}</td>)}</tr>)}</tbody></table>{!table.rows.length&&<p className={s.empty}>Nenhum registro nesta etapa.</p>}</div><div className={s.pagination}><button disabled={safePage===0} onClick={()=>setPage(p=>p-1)}>Anterior</button><span>{safePage+1} / {pages}</span><button disabled={safePage>=pages-1} onClick={()=>setPage(p=>p+1)}>Próxima</button></div></section>;
}
...
export default function DataFlowLab({userId='',demo=false}:{userId?:string;demo?:boolean}) {
  ...
  const mission=run?.name==='Caso Aurora · vendas'&&run.config.dedup&&run.config.filter&&counts[2]===20&&counts[3]===20;
  return <main className={s.app}>
    ...
    <div className={s.workspace}>
      <aside className={s.sidebar}><div className={s.sideTitle}><Layers size={17}/><h2>Seu pipeline</h2><span>01—05</span></div>
        ...
        <div className={s.mission}><span className={s.eyebrow}>MISSÃO / AURORA</span><h3>O faturamento fantasma</h3><p>Elimine a multiplicação de pedidos e mantenha apenas vendas aprovadas. Execute e compare.</p>{mission?<strong><CheckCircle2 size={16}/> Fluxo corrigido: 20 pedidos válidos!</strong>:<small>Dica: investigue a chave cliente_id e o status.</small>}<button disabled={busy} onClick={()=>{const fresh=sample();setData(fresh);setConfig({...DEFAULT_CONFIG});setSql(DEFAULT_SQL);setName('Caso Aurora · vendas');setDirty(true);setError('');}}>Carregar caso de exemplo</button></div>
      </aside>
