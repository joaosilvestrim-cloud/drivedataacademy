...
export default function VideoCarousel({ ids }: { ids: string[] }) {
  ...
  function go(idx: number) {
    ...
    setI(idx);
    setPlaying(false); // mostra a capa até o novo vídeo tocar
      ...
      <div className="relative">
        <div className="glow-border overflow-hidden rounded-[1.75rem]">
          <div className="relative aspect-video w-full overflow-hidden rounded-[1.75rem] bg-ink-900">
            ...
            <div
              className={`absolute inset-0 z-20 grid place-items-center bg-ink-900 transition-opacity duration-500 ${
                playing ? "pointer-events-none opacity-0" : "opacity-100"
              }`}
              aria-hidden
            >
              <span className="h-9 w-9 animate-spin rounded-full border-2 border-white/15 border-t-brand-green" />
            </div>
          </div>
        </div>
