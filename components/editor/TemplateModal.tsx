        ...
        <div className="flex flex-col gap-4 p-4 sm:flex-row">
          ...
          <div className="sm:w-56 sm:shrink-0">
            <div className="flex h-40 items-center justify-center overflow-hidden rounded-xl border border-border bg-[#0b1220] sm:h-full sm:min-h-[200px]">
              <iframe title={tr("Prévia")} sandbox="" srcDoc={buildPreview(doc)} className="h-full w-full" style={{ background: "transparent", pointerEvents: "none" }} />
            </div>
            <p className="mt-1.5 text-center text-[11px] text-muted">{tr("É assim que o template vai aparecer.")}</p>
          </div>

          {/* Campos */}
          <div className="flex min-w-0 flex-1 flex-col gap-3">
            <label className="flex flex-col gap-1 text-sm">
              <span className="font-medium">{tr("Nome")} <span className="text-red-500">*</span></span>
              <input value={nome} onChange={(e) => setNome(e.target.value)} placeholder={tr("Ex.: Card de faturamento")} className="rounded-md border border-border bg-surface px-3 py-2 focus:border-viz focus:outline-none" />
            </label>
            <label className="flex flex-col gap-1 text-sm">
              <span className="font-medium">{tr("Descrição")} <span className="text-muted">(opcional)</span></span>
              <textarea value={desc} onChange={(e) => setDesc(e.target.value)} rows={2} placeholder={tr("Uma linha sobre para que serve o template.")} className="resize-none rounded-md border border-border bg-surface px-3 py-2 focus:border-viz focus:outline-none" />
            </label>
            <label className="flex items-center gap-2 text-sm">
              <input type="checkbox" checked={premium} onChange={(e) => setPremium(e.target.checked)} className="h-4 w-4 accent-viz" />
              <span>{tr("Exclusivo de assinantes")} <span className="text-muted">(premium)</span></span>
            </label>
