      ...
      {ok ? (
        ...
        <div className="flex flex-col gap-4 p-4 sm:flex-row">
          ...
          <div className="sm:w-56 sm:shrink-0">
            ...
            <p className="mt-1.5 text-center text-[11px] text-muted">{tr("É assim que vai aparecer.")}</p>
          </div>
          ...
          <div className="flex min-w-0 flex-1 flex-col gap-3">
            <label className="flex flex-col gap-1 text-sm">
              ...
              <input value={nome} onChange={(e) => setNome(e.target.value)} placeholder={tr("Ex.: Card de faturamento")} className="rounded-md border border-border px-3 py-2 focus:border-viz focus:outline-none" />
            </label>
            <label className="flex flex-col gap-1 text-sm">
              <span className="font-medium">{tr("Descrição")} <span className="text-muted">(opcional)</span></span>
              <textarea value={desc} onChange={(e) => setDesc(e.target.value)} rows={2} placeholder={tr("Conte em uma linha para que serve esse visual.")} className="resize-none rounded-md border border-border px-3 py-2 focus:border-viz focus:outline-none" />
            </label>
