...
function Engraving({rank}:{rank:number}){
  ...
  return <svg className={s.engraving} viewBox="0 0 200 200" aria-hidden="true">
    ...
    <text className={s.arcText}><textPath href={`#arc-${id}`} startOffset="50%" textAnchor="middle">{tr("DRIVEDATA ACADEMY")}</textPath></text>
...
function Coin({rank,rotation}:{rank:number;rotation?:{x:number;y:number}}){
  ...
  return <div className={`${s.coin} ${s[tier.key]} ${rotation?s.controlled:s.idle}`} style={rotation?{transform:`rotateX(${rotation.x}deg) rotateY(${rotation.y}deg)`}:undefined} aria-hidden="true">
    ...
    <div className={s.reverse}><div className={s.reverseFace}><Image src="/drivedata-symbol.png" alt="" width={56} height={56}/><strong>{tr("DriveData")}</strong><span>ACADEMY</span><i>{tr("APRENDER · COMPARTILHAR")}<br/>TRANSFORMAR</i></div></div>
  </div>;
}
...
export default function RankMedal({rank,name,points,compact=false}:{rank:number;name?:string;points?:number;compact?:boolean}){
  ...
  return <>
    <button ref={trigger} type="button" className={`${s.medalButton} ${s[tier.key]} ${compact?s.compact:''}`} aria-label={`Explorar medalha ${tier.name} de ${rank}º lugar${name?` de ${name}`:''}`} onClick={()=>{setRotation({x:-10,y:-18});setOpen(true);}}>
      ...
      {!compact&&<span className={s.explore}>{tr("EXPLORAR MEDALHA")} <span>↗</span></span>}
    </button>
    <dialog ref={dialog} className={`${s.dialog} ${s[tier.key]}`} aria-labelledby={id} onCancel={e=>{e.preventDefault();close();}} onClose={()=>setOpen(false)}>
      {open&&<><button type="button" className={s.close} aria-label={tr("Fechar medalha")} onClick={close}><X size={20}/></button>
        <p className={s.kicker}>{tr("DRIVEDATA ACADEMY · COMUNIDADE")}</p><h2 id={id}>{tr("Medalha de")} {tier.name}</h2><p className={s.caption}>{tier.caption}</p>
        <div className={s.showcase}><div className={s.orbit} aria-hidden="true"/><div className={s.interactive} tabIndex={0} role="group" aria-label={tr("Medalha 3D. Arraste ou use as setas para girar.")} onPointerDown={e=>{if(e.button!==0)return;e.currentTarget.setPointerCapture(e.pointerId);drag.current={x:e.clientX,y:e.clientY,rx:rotation.x,ry:rotation.y};}} onPointerMove={move} onPointerUp={()=>{drag.current=null;}} onPointerCancel={()=>{drag.current=null;}} onLostPointerCapture={()=>{drag.current=null;}} onKeyDown={e=>{if(['ArrowLeft','ArrowRight','ArrowUp','ArrowDown'].includes(e.key)){e.preventDefault();setRotation(r=>({x:Math.max(-60,Math.min(60,r.x+(e.key==='ArrowUp'?-10:e.key==='ArrowDown'?10:0))),y:r.y+(e.key==='ArrowLeft'?-20:e.key==='ArrowRight'?20:0)}));}}}><Coin rank={rank} rotation={rotation}/></div></div>
        <p className={s.instruction}>{tr("Arraste para girar · use as setas do teclado")}</p>
        <div className={s.actions}><button type="button" onClick={()=>setRotation(r=>({...r,y:r.y+180}))}><RotateCw size={15}/>{tr("Ver outro lado")}</button><button type="button" onClick={()=>setRotation({x:-10,y:-18})}><RotateCcw size={15}/>{tr("Restaurar")}</button></div>
        ...
        <p className={s.disclaimer}>{tr("Representa a posição atual no ranking. A medalha acompanha as mudanças de colocação; não é um prêmio permanente.")}</p>
      </>}
    </dialog>
  </>;
}
