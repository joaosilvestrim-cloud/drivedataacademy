...
export default function RaioX({nome="Raio-X do Dashboard",historicoInicial=[],demoInicial=false}:{nome?:string;historicoInicial?:ResumoHistorico[];demoInicial?:boolean}) {
  ...
  function analisar(file:File) {
    ...
    setErro("");setMensagem("");
    if(!/\.(pbix|pbit)$/i.test(file.name)||file.size>LIMITE_ARQUIVO||!file.size){setErro(tr("Escolha um .pbix ou .pbit válido de até 300 MB. Para reduzir o tamanho, exporte como .pbit."));return;}
    try {
      const w=new Worker(new URL("../../../../lib/raiox/analise.worker.ts",import.meta.url));
      workerRef.current=w;setEtapa(tr("Preparando a análise local"));
      timerRef.current=setTimeout(()=>{parar();setErro(tr("A leitura demorou mais de um minuto. Tente exportar o relatório como .pbit ou dividir o arquivo."));},60000);
      ...
      w.onerror=()=>{if(workerRef.current!==w)return;parar();setErro(tr("A análise foi interrompida. Tente novamente com um arquivo menor ou exportado como .pbit."));};
      w.postMessage({arquivo:file});
    } catch {parar();setErro(tr("Seu navegador não conseguiu iniciar a análise. Recarregue a página e tente novamente."));}
  }
  ...
  async function guardar() {
    ...
    } catch {setErro(tr("O salvamento foi interrompido. Tente novamente; a mesma versão não será duplicada."));} finally{trava.current=false;setOcupado(false);}
  }
  async function abrir(id:string,comparar=false) {
    ...
    try {const r=await abrirLaudo(id);if(!r.ok){setErro(r.erro);return;}if(comparar){if(!ativo||!compararLaudos(ativo,r.salvo)){setErro(tr("Compare versões diferentes do mesmo projeto, com as mesmas dimensões avaliadas e a mesma versão de regras."));return;}setAnterior(r.salvo);setAba("diagnostico");}else mostrar(r.salvo);
    }catch{setErro(tr("Não foi possível carregar o histórico. Tente novamente."));}finally{trava.current=false;setOcupado(false);}
  }
  async function atualizarPlano(plano:Plano) {
    if(!ativo||trava.current)return;
    if(!ativo.id||demo){setAtivo({...ativo,plano});setMensagem(tr("Plano atualizado nesta sessão. Salve o diagnóstico ou exporte para conservar sua revisão."));return;}
    trava.current=true;setOcupado(true);setErro("");
    try{const r=await salvarPlano(ativo.id,plano);if(r.ok){setAtivo({...ativo,plano});setMensagem(tr("Plano salvo. Os ajustes serão confirmados por uma nova análise."));}else setErro(r.erro);}catch{setErro(tr("Não foi possível guardar a alteração do plano. Tente novamente."));}finally{trava.current=false;setOcupado(false);}
  }
  ...
  return <div className={s.root}>
    <div className={s.screenOnly}>
      ...
      <div className={s.introActions}>{ativo&&<button disabled={busy} onClick={()=>setEntrada(!entrada)}><Upload size={16}/>{entrada?tr("Fechar seleção"):tr("Analisar outra versão")}</button>}<button onClick={exemplo} disabled={busy}><ScanLine size={16}/>{tr("Explorar exemplo")}</button><button onClick={()=>setAba("historico")} disabled={busy}><History size={16}/>{tr("Histórico")} <span>{historico.length}</span></button></div>
      ...
      {etapa&&<div className={s.processing} role="status"><Loader2 size={20} className={s.spin}/><span>{etapa}…</span><button onClick={()=>{parar();setMensagem(tr("Análise cancelada. O resultado anterior foi mantido."));}}>{tr("Cancelar")}</button></div>}
    ...
    {laudo&&ativo&&<div ref={resultado} tabIndex={-1} className={s.result}>
      <section className={s.score} data-tour="raiox-nota"><div className={s.scoreRing} style={{"--score":`${laudo.nota}%`} as React.CSSProperties}><strong>{laudo.nota}</strong><span>{tr("de 100")}</span></div><div className={s.scoreText}><p className={s.eyebrow}>{laudo.parcial?tr("COBERTURA DO RELATÓRIO"):tr("RELATÓRIO E MODELO")} · REGRAS {laudo.versao||"1.0.0"}</p><h2>{ativo.projeto}</h2><p>{laudo.arquivo} · {laudo.resumo.paginas} páginas · {laudo.resumo.visuais} visuais · {laudo.achados.length} {tr("pontos de atenção")}</p><small>{tr("A nota resume as regras aplicáveis. Não certifica os números de negócio nem mede o tempo de execução.")}</small></div><div className={`${s.scoreActions} ${s.screenOnly}`}><button disabled={busy||demo||!!ativo.id} className={s.primary} onClick={guardar}><Check size={16}/>{ativo.id&&!demo?tr("No histórico"):tr("Salvar diagnóstico")}</button><button onClick={()=>baixar("raio-x-diagnostico.json",JSON.stringify(ativo,null,2),"application/json")}><Download size={16}/>{tr("Exportar JSON")}</button><button onClick={()=>window.print()}>{tr("Imprimir / PDF")}</button></div></section>
      ...
      <nav className={`${s.tabs} ${s.screenOnly}`} aria-label={tr("Seções do diagnóstico")}>{([['diagnostico',tr("Explorar diagnóstico")],['plano',`Plano de ação (${itensPlano.length})`],['historico',tr("Histórico e comparação")]] as const).map(([id,label])=><button key={id} aria-pressed={aba===id} onClick={()=>setAba(id)}>{label}</button>)}</nav>
