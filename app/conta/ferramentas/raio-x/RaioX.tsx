...
export default function RaioX({nome="Raio-X do Dashboard",historicoInicial=[],demoInicial=false}:{nome?:string;historicoInicial?:ResumoHistorico[];demoInicial?:boolean}) {
  ...
  return <div className={s.root}>
    <div className={s.screenOnly}>
      <Link href="/conta/ferramentas" className={s.back}><ArrowLeft size={15}/> Todas as ferramentas</Link>
      <header className={s.header}><div><p className={s.eyebrow}>POWER BI / DIAGNÓSTICO</p><h1>{nome}</h1><p className={s.subtitle}>Encontre pontos de atenção, entenda as evidências e organize sua próxima revisão.</p></div><span className={s.private}><ShieldCheck size={18}/> Análise no navegador</span></header>
      <div className={s.introActions}>{ativo&&<button disabled={busy} onClick={()=>setEntrada(!entrada)}><Upload size={16}/>{entrada?"Fechar seleção":"Analisar outra versão"}</button>}<button onClick={exemplo} disabled={busy}><ScanLine size={16}/>Explorar exemplo</button><button onClick={()=>setAba("historico")} disabled={busy}><History size={16}/>Histórico <span>{historico.length}</span></button></div>
      {demo&&<p className={s.demo}>Demonstração com dados fictícios. As interações do exemplo ficam apenas nesta sessão.</p>}
      {entrada&&<section className={`${s.upload} ${arrastando?s.dragging:""}`} data-tour="raiox-upload" onDragOver={e=>{e.preventDefault();if(!busy)setArrastando(true);}} onDragLeave={()=>setArrastando(false)} onDrop={e=>{e.preventDefault();setArrastando(false);if(!busy&&e.dataTransfer.files[0])analisar(e.dataTransfer.files[0]);}}>
        <div className={s.uploadIcon}><FileSearch size={27}/></div><h2>Um novo olhar para o seu relatório</h2><p>Arraste seu arquivo aqui ou escolha no computador.</p>
        ...
        <button className={s.primary} disabled={busy} onClick={()=>input.current?.click()}><Upload size={17}/>{etapa?"Analisando…":"Escolher .pbix ou .pbit"}</button><small>Até 300 MB. Para relatórios grandes, prefira .pbit.</small>
        <label className={s.projectInput}>Projeto para comparar versões<input value={projeto} onChange={e=>setProjeto(e.target.value)} maxLength={100} disabled={busy} placeholder="Ex.: Vendas da empresa (opcional)"/><small>Use o mesmo nome nas próximas análises. Se ficar vazio, usamos o nome do arquivo.</small></label>
        <div className={s.formats} data-tour="raiox-pbit"><div><b>.pbix</b><span>Páginas, visuais e organização.</span></div><div><b>.pbit</b><span>Inclui também o modelo e as medidas legíveis.</span></div></div>
        <p className={s.privacy}>O arquivo não é enviado. Ao escolher salvar, o laudo guarda nomes, achados e o mapa estrutural. Fórmulas e linhas de dados não fazem parte do histórico.</p>
      </section>}
      {etapa&&<div className={s.processing} role="status"><Loader2 size={20} className={s.spin}/><span>{etapa}…</span><button onClick={()=>{parar();setMensagem("Análise cancelada. O resultado anterior foi mantido.");}}>Cancelar</button></div>}
    ...
    {laudo&&ativo&&<div ref={resultado} tabIndex={-1} className={s.result}>
      <section className={s.score} data-tour="raiox-nota"><div className={s.scoreRing} style={{"--score":`${laudo.nota}%`} as React.CSSProperties}><strong>{laudo.nota}</strong><span>de 100</span></div><div className={s.scoreText}><p className={s.eyebrow}>{laudo.parcial?"COBERTURA DO RELATÓRIO":"RELATÓRIO E MODELO"} · REGRAS {laudo.versao||"1.0.0"}</p><h2>{ativo.projeto}</h2><p>{laudo.arquivo} · {laudo.resumo.paginas} páginas · {laudo.resumo.visuais} visuais · {laudo.achados.length} pontos de atenção</p><small>A nota resume as regras aplicáveis. Não certifica os números de negócio nem mede o tempo de execução.</small></div><div className={`${s.scoreActions} ${s.screenOnly}`}><button disabled={busy||demo||!!ativo.id} className={s.primary} onClick={guardar}><Check size={16}/>{ativo.id&&!demo?"No histórico":"Salvar diagnóstico"}</button><button onClick={()=>baixar("raio-x-diagnostico.json",JSON.stringify(ativo,null,2),"application/json")}><Download size={16}/>Exportar JSON</button><button onClick={()=>window.print()}>Imprimir / PDF</button></div></section>
      ...
      {laudo.parcial&&<p className={`${s.coverage} ${s.screenOnly}`}>Modelo e DAX não foram avaliados por completo. No Power BI, use <b>Arquivo → Exportar → Modelo do Power BI (.pbit)</b>. O template pode conter nomes e valores nos metadados, embora não inclua as linhas do modelo.</p>}
      <nav className={`${s.tabs} ${s.screenOnly}`} aria-label="Seções do diagnóstico">{([['diagnostico','Explorar diagnóstico'],['plano',`Plano de ação (${itensPlano.length})`],['historico','Histórico e comparação']] as const).map(([id,label])=><button key={id} aria-pressed={aba===id} onClick={()=>setAba(id)}>{label}</button>)}</nav>
      {comparacao&&<section className={`${s.comparison} ${s.screenOnly}`}><div><p className={s.eyebrow}>MESMO PROJETO · MESMA COBERTURA</p><h3>{comparacao.diferenca>=0?"+":""}{comparacao.diferenca} pontos em relação a {data(anterior!.criadoEm)}</h3><p>{comparacao.resolvidos.length} resolvidos · {comparacao.persistentes.length} persistentes · {comparacao.novos.length} novos</p></div><button aria-label="Fechar comparação" onClick={()=>setAnterior(null)}><X size={17}/></button>{comparacao.resolvidos.length>0&&<details><summary>Ver o que deixou de aparecer</summary><ul>{comparacao.resolvidos.map(a=><li key={chaveAchado(a)}>{a.titulo} — {a.onde}</li>)}</ul></details>}</section>}

      {aba==="diagnostico"&&<div className={s.screenOnly}>
        <div className={s.filters}><label>Buscar no diagnóstico<input type="search" placeholder="Página, medida ou problema" value={busca} onChange={e=>setBusca(e.target.value)}/></label><label>Dimensão<select value={dimensao} onChange={e=>setDimensao(e.target.value)}><option value="">Todas</option>{Object.entries(NOME_DIMENSAO).map(([v,n])=><option key={v} value={v}>{n}</option>)}</select></label><label>Prioridade<select value={prioridade} onChange={e=>setPrioridade(e.target.value)}><option value="">Todas</option>{Object.entries(severidade).map(([v,n])=><option key={v} value={v}>{n}</option>)}</select></label></div>
        <div className={s.workspace}>
          <div>
            <section className={s.map}><div className={s.mapHead}><h3>Mapa do relatório</h3>{pagina&&<label><span className={s.srOnly}>Página</span><select value={pagina.id} onChange={e=>{setPaginaId(e.target.value);setVisualId("");}}>{laudo.paginas?.map(p=><option key={p.id} value={p.id}>{p.nome}</option>)}</select></label>}</div>
              ...
              </div><label className={s.visualSelect}>Localizar visual<select value={visualId} onChange={e=>escolherVisual(e.target.value)}><option value="">Escolha um elemento</option>{pagina.visuais.map((v,i)=><option value={v.id} key={v.id}>{i+1}. {nomeDoVisual(v.tipo)}{v.oculto?" (oculto)":""}</option>)}</select></label><p className={s.mapNote}>{visualId&&!laudo.achados.some(a=>a.alvo?.paginaId===pagina.id&&a.alvo.visuais.includes(visualId))?"Nenhum achado localizado neste elemento pelas regras aplicadas.":"Selecione um elemento para localizar seus achados. O mapa mostra a estrutura, não os gráficos reais."}</p></>:<p className={s.empty}>Este laudo antigo não possui mapa. Analise o arquivo novamente para gerar uma versão atualizada.</p>}
            </section>
            <div className={s.listHeading}><h3>Pontos de atenção</h3><span>{lista.length} de {laudo.achados.length}</span></div>
            <div className={s.findings} data-tour="raiox-achados">{lista.map(a=><button key={chaveAchado(a)} aria-pressed={achado===a} onClick={()=>escolher(chaveAchado(a))}><span className={s.severity} data-level={a.severidade}/><span><b>{a.titulo}</b><small>{NOME_DIMENSAO[a.dimensao]} · {severidade[a.severidade]} · {a.onde}</small></span><ChevronRight size={16}/></button>)}{!lista.length&&<p className={s.empty}>{laudo.achados.length?"Nenhum ponto corresponde aos filtros.":"Nenhum ponto de atenção nas regras aplicadas. Confira a cobertura: isso não certifica todos os aspectos do relatório."}</p>}</div>
          </div>
          <section ref={detalhe} className={s.inspector} aria-label="Detalhes do achado"><p className={s.eyebrow}>INSPECIONAR E REVISAR</p>{achado?<><span className={s.badge} data-level={achado.severidade}>{severidade[achado.severidade]} · {NOME_DIMENSAO[achado.dimensao]}</span><h3 aria-live="polite">{achado.titulo}</h3><p className={s.location}>{achado.onde}</p><h4>Por que revisar</h4><p>{achado.porque}</p><h4>Como revisar no Power BI</h4><p className={s.guidance}>{achado.comoArrumar}</p><button className={s.primary} disabled={busy} onClick={()=>{const k=chaveAchado(achado);const novo={...plano};if(novo[k])delete novo[k];else novo[k]="pendente";void atualizarPlano(novo);}}><ListChecks size={17}/>{plano[chaveAchado(achado)]?"Remover do plano":"Adicionar ao plano"}</button><small>A marcação manual organiza a revisão. A nota só muda após uma nova análise.</small>{achado.regra==="sem_tabela_datas"&&<Link href="/conta/ferramentas/forja" className={s.related}>Abrir a Forja DAX <ArrowRight size={15}/></Link>}<Link href="/conta/comunidade/power-bi" className={s.related}>Tirar uma dúvida na comunidade <ArrowRight size={15}/></Link></>:<p className={s.empty}>Selecione um ponto de atenção para ver sua evidência e orientação.</p>}</section>
        </div>
      </div>}
      {aba==="plano"&&<section className={`${s.plan} ${s.screenOnly}`}><p className={s.eyebrow}>DA ANÁLISE À AÇÃO</p><h2>Sua próxima revisão</h2><p>{itensPlano.filter(a=>plano[chaveAchado(a)]==="ajustado").length} de {itensPlano.length} ajustes marcados. A confirmação depende de uma nova análise.</p>{itensPlano.map(a=><div className={s.planRow} key={chaveAchado(a)}><div><b>{a.titulo}</b><small>{a.onde}</small></div><label><span className={s.srOnly}>Estado de {a.titulo}</span><select disabled={busy} value={plano[chaveAchado(a)]} onChange={e=>void atualizarPlano({...plano,[chaveAchado(a)]:e.target.value as EstadoRevisao})}><option value="pendente">Pendente</option><option value="revisando">Em revisão</option><option value="ajustado">Ajustado — aguarda análise</option></select></label><button disabled={busy} aria-label={`Remover ${a.titulo} do plano`} onClick={()=>{const novo={...plano};delete novo[chaveAchado(a)];void atualizarPlano(novo);}}><X size={17}/></button></div>)}{!itensPlano.length&&<p className={s.empty}>Abra os achados do diagnóstico e escolha “Adicionar ao plano”.</p>}{!ativo.id&&<p className={s.mapNote}>Este plano está apenas nesta sessão. Salve o diagnóstico ou exporte o JSON para conservá-lo.</p>}</section>}
      <section className={s.printOnly}><h2>Diagnóstico e plano de revisão</h2>{laudo.notas.map(n=><p key={n.dimensao}>{NOME_DIMENSAO[n.dimensao]}: {n.completa?n.nota:n.motivo||"Não avaliado"}</p>)}{laudo.achados.map(a=><article key={chaveAchado(a)}><h3>{a.titulo}</h3><p>{a.onde} · {severidade[a.severidade]}</p><p>{a.porque}</p><p>{a.comoArrumar}</p>{plano[chaveAchado(a)]&&<p>Revisão: {plano[chaveAchado(a)]} (informada pelo usuário)</p>}</article>)}</section>
    </div>}
    {aba==="historico"&&<section className={`${s.history} ${s.screenOnly}`} data-tour="raiox-historico"><p className={s.eyebrow}>SEU TRABALHO, VERSÃO A VERSÃO</p><h2>Histórico de diagnósticos</h2><p>Últimos 30 laudos. Compare versões do mesmo projeto com a mesma cobertura e regras.</p>{historico.map(h=><div className={s.historyRow} key={h.id}><strong>{h.nota}</strong><div><b>{h.projeto}</b><small>{h.arquivo} · {data(h.criadoEm)} · {h.parcial?"relatório":"relatório e modelo"} · v{h.versao}</small></div><button disabled={busy} onClick={()=>void abrir(h.id)}>Abrir</button>{ativo&&!demo&&ativo.id!==h.id&&ativo.projeto.trim().toLocaleLowerCase()===h.projeto.trim().toLocaleLowerCase()&&ativo.laudo.versao===h.versao&&<button disabled={busy} onClick={()=>void abrir(h.id,true)}>Comparar</button>}</div>)}{!historico.length&&<p className={s.empty}>Você ainda não tem diagnósticos salvos. Ao terminar uma análise, escolha “Salvar diagnóstico”.</p>}</section>}
    <details className={`${s.help} ${s.screenOnly}`}><summary>Como interpretar o diagnóstico e proteger os metadados</summary><p>As regras procuram sinais em páginas, visuais, modelo e DAX. Achados são orientações de revisão, não uma validação dos resultados de negócio. Não executamos consultas nem medimos desempenho real.</p><p>PBIX permite revisar a estrutura do relatório. PBIT pode disponibilizar também o modelo. Dimensões sem evidência ficam sem nota. Para exportar um PBIT, abra Arquivo → Exportar → Modelo do Power BI.</p><p>A análise acontece no navegador. Você escolhe se deseja salvar o laudo, que contém nomes e o mapa estrutural. Se esses metadados forem confidenciais, mantenha a análise local. Nenhuma fórmula ou linha de dados é enviada no salvamento.</p><p>Para comparar versões, informe o mesmo nome de projeto. A comparação exige as mesmas dimensões avaliadas e a mesma versão das regras. Uma análise local registra participação autodeclarada, não proficiência comprovada.</p></details>
  </div>;
}
