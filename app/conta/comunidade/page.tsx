...
const fallbackStyle = { icon: "📌", from: "#3b9dff", to: "#22d3ee" };
      ...
      <div className="glass overflow-hidden rounded-3xl border border-white/8">
        <div className="flex flex-wrap items-center justify-between gap-4 bg-gradient-to-r from-brand-green/[0.08] to-brand-blue/[0.05] p-6">
          ...
          <div className="flex items-center gap-6">
            ...
            <Link href="/conta/ranking" className="rounded-xl bg-white/10 px-4 py-2.5 text-sm font-semibold text-white transition-colors hover:bg-white/15">🏆 Ranking</Link>
          </div>
        </div>
      </div>
        ...
        {(channels ?? []).map((c: any) => {
          ...
          return (
            <Link key={c.id} href={`/conta/comunidade/${c.slug}`} className="card-hover glass flex items-start gap-4 rounded-2xl border border-white/8 p-5">
              <span className="grid h-12 w-12 shrink-0 place-items-center rounded-xl text-2xl text-ink-900" style={{ backgroundImage: `linear-gradient(135deg, ${s.from}, ${s.to})` }}>{s.icon}</span>
