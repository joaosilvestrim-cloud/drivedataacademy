...
export default function AssinarForm({ turmaNome = "Ferramenta de Visuais" }: { turmaNome?: string }) {
  ...
  return (
    <form onSubmit={onSubmit} className="space-y-4">
      <input name="cpf" inputMode="numeric" placeholder={tr("CPF (para emitir a cobrança)")} className={field} />
      <input name="phone" placeholder={tr("WhatsApp com DDD (opcional)")} className={field} />
      {result && !result.ok && (
