...
import { Field, TextareaField, SelectField, CheckboxField, FormSection, FormActions } from '@/components/ui/form';
...
export default function KnowledgeAdmin({initial,revision:initialRevision,courses,versions,students=[],practical=[]}:{initial:KnowledgeDocument;revision:number;courses:{id:string;title:string;slug:string}[];versions:{id:string;published_at:string}[];students?:{id:string;full_name:string|null}[];practical?:{id:string;userId:string;label:string;at:string}[]}) {
  ...
  return <div className="space-y-6"><div className="flex flex-wrap items-start justify-between gap-4"><div><p className="text-xs uppercase tracking-widest text-brand-green">Administração acadêmica</p><h1 className="mt-1 text-3xl font-semibold">Knowledge Universe 4D</h1><p className="mt-2 text-sm text-slate-400">Defina competências, treinamentos e critérios de domínio. Nenhuma IA participa dos cálculos.</p></div><Link href="/universo" className={secondary}>Ver meu universo ↗</Link></div>
      ...
      <form className="flex flex-col gap-5" onSubmit={e=>{e.preventDefault();const f=new FormData(e.currentTarget);mutate(d=>{const i=d.areas.length;const angle=i*2.4;const position:Vec3=[Math.cos(angle)*4,Math.sin(angle)*3,0];d.areas.push({id:crypto.randomUUID(),name:str(f,'name'),color:str(f,'color'),position});});e.currentTarget.reset();}}>
        <FormSection title="Nova macroárea">
          <div className="grid items-end gap-4 tablet:grid-cols-[1fr_8rem_auto]">
            <Field scope="area" name="name" label="Nome" required maxLength={160}/>
      ...
      <form key={editing||'new'} className="flex flex-col gap-5" onSubmit={competency}>
        <FormSection title={selected?'Editar competência':'Nova competência'}>
          <div className="grid gap-4 tablet:grid-cols-2">
            <Field scope="competencia" name="name" label="Nome" required maxLength={160} defaultValue={selected?.name}/>
            ...
            <Field scope="competencia" name="freshness" label="Meia-vida da atualidade" type="number" min="1" max="3650" required defaultValue={selected?.halfLifeDays??180} description="Em dias."/>
          </div>
          ...
          <fieldset className="flex flex-col gap-2">
            ...
            <div className="mt-1 grid gap-4 tablet:grid-cols-3 lg:grid-cols-5">
              {(Object.keys(WEIGHTS) as Dimension[]).map(k=><Field key={k} scope="competencia" name={k} label={LABELS[k]} type="number" min="1" max="10000" required defaultValue={selected?.targets[k]??100}/>)}
            </div>
          </fieldset>
        </FormSection>
        <FormActions>
      ...
      <FormSection title="Competências de cada treinamento" description="Os pesos do curso devem somar 100%. Créditos são unidades pedagógicas, e grupos equivalentes evitam somar novamente o mesmo conhecimento.">
        ...
        <form key={courseId} className="flex flex-col gap-4" onSubmit={e=>{e.preventDefault();const f=new FormData(e.currentTarget);const competency=str(f,'competency');mutate(d=>{d.mappings=d.mappings.filter(m=>!(m.courseId===courseId&&m.competency===competency));d.mappings.push({courseId,competency,weight:num(f,'weight')/100,credits:num(f,'credits'),group:str(f,'group'),advanced:f.get('advanced')==='on'});});}}>
          <div className="grid gap-4 tablet:grid-cols-2">
            ...
            <Field scope="mapa" name="weight" label="Peso" type="number" min=".01" max="100" step=".01" defaultValue="100" required description="Em porcentagem do curso."/>
            <Field scope="mapa" name="credits" label="Créditos do curso" type="number" min="1" max="10000" defaultValue="100" required/>
            <Field scope="mapa" name="group" label="Grupo de conhecimento" defaultValue="fundamentos" required maxLength={80} description="Mesmo nome significa conteúdos equivalentes."/>
          </div>
      ...
      <form className="flex flex-col gap-4" onSubmit={e=>{e.preventDefault();const f=new FormData(e.currentTarget);mutate(d=>{d.weights=Object.fromEntries(Object.keys(WEIGHTS).map(k=>[k,num(f,k)])) as Record<Dimension,number>;});}}>
        <FormSection title="Composição dos 100 pontos">
          <div className="grid gap-4 tablet:grid-cols-3 lg:grid-cols-5">
            {(Object.keys(WEIGHTS) as Dimension[]).map(k=><Field key={k} scope="pesos" name={k} label={LABELS[k]} type="number" min="0" max="100" defaultValue={doc.weights?.[k]??WEIGHTS[k]}/>)}
          </div>
      ...
      <form className="flex flex-col gap-4" onSubmit={e=>{e.preventDefault();const f=new FormData(e.currentTarget);mutate(d=>{d.relations.push({source:str(f,'source'),target:str(f,'target'),strength:num(f,'strength')/100});});}}>
        <FormSection title="Relações entre competências">
          <div className="grid gap-4 tablet:grid-cols-3">
            ...
            <Field scope="relacao" name="strength" label="Afinidade" type="number" min="0" max="100" defaultValue="80" description="Em porcentagem."/>
          </div>
      ...
      <form className="flex flex-col gap-4" onSubmit={e=>{e.preventDefault();const f=new FormData(e.currentTarget),target=str(f,'target'),required=str(f,'required');mutate(d=>{const existing=d.unlocks.find(u=>u.target===target);const rules=existing?requirements(existing.rule).filter(r=>r.competency!==required):[];rules.push({competency:required,minimum:num(f,'minimum')});d.unlocks=d.unlocks.filter(u=>u.target!==target);d.unlocks.push({target,rule:{all:rules}});});}}>
        <FormSection title="Pré-requisitos" description="Todos os requisitos listados para uma competência são obrigatórios.">
          <div className="grid gap-4 tablet:grid-cols-3">
            ...
            <Field scope="prereq" name="minimum" label="Score mínimo" type="number" min="1" max="100" defaultValue="60" required/>
          </div>
    ...
    {tab==='pratica'&&<form className="flex flex-col gap-5" onSubmit={async e=>{e.preventDefault();const f=new FormData(e.currentTarget);const id=requestId||crypto.randomUUID();setRequestId(id);setBusy(true);const r=await recordPracticalEvidence({userId:str(f,'student'),competency:str(f,'competency'),dimension:str(f,'dimension'),group:str(f,'group'),units:num(f,'units'),quality:num(f,'quality')/100,advanced:f.get('advanced')==='on',label:str(f,'label'),requestId:id});setBusy(false);setNotice(r.ok?'Evidência registrada. O aluno verá o resultado ao atualizar seu universo.':r.error);if(r.ok)setRequestId('');}}>
      <FormSection title="Registrar evidência revisada pelo instrutor" description="Use somente para atividades verificadas. O registro entra no histórico com a data atual; grupos equivalentes não acumulam pontos repetidos.">
        <div className="grid gap-4 tablet:grid-cols-2">
          <SelectField scope="evidencia" name="student" label="Aluno" required>{students.map(s=><option key={s.id} value={s.id}>{s.full_name||'Aluno sem nome'}</option>)}</SelectField>
          <SelectField scope="evidencia" name="competency" label="Competência publicada" required>{options}</SelectField>
          <SelectField scope="evidencia" name="dimension" label="Tipo">
            <option value="exercise">Exercício</option>
            <option value="challenge">Desafio prático</option>
            <option value="retention">Revisão espaçada</option>
          </SelectField>
          <Field scope="evidencia" name="group" label="Referência / grupo de equivalência" required maxLength={80}/>
          <Field scope="evidencia" name="units" label="Unidades demonstradas" type="number" min="0" max="10000" defaultValue="100" required/>
          <Field scope="evidencia" name="quality" label="Qualidade da evidência" type="number" min="0" max="100" defaultValue="100" required description="Em porcentagem."/>
          <Field scope="evidencia" name="label" label="Descrição da entrega verificada" required maxLength={240} className="tablet:col-span-2"/>
        </div>
        <CheckboxField scope="evidencia" name="advanced" label="Atividade de nível avançado"/>
      </FormSection>
      <FormActions>
        <Button type="submit" disabled={busy||!versions.length}>Registrar evidência</Button>
      </FormActions>
    </form>}
    {tab==='acessos'&&<div className="flex flex-col gap-10">
      <form className="flex flex-col gap-5" onSubmit={async e=>{e.preventDefault();const f=new FormData(e.currentTarget);setBusy(true);const r=await grantKnowledgeAccess(str(f,'email'),str(f,'expires')?new Date(str(f,'expires')+'T23:59:59').toISOString():null);setBusy(false);setNotice(r.ok?'Acesso liberado para o aluno.':r.error);}}>
        <FormSection title="Acesso ao produto" description="Administradores e assinantes ativos da Academy têm acesso. Você também pode conceder acesso individual.">
          <div className="grid gap-4 tablet:grid-cols-2">
            <Field scope="acesso" name="email" label="E-mail da conta do aluno" type="email" required/>
            <Field scope="acesso" name="expires" label="Válido até" type="date" description="Em branco, o acesso não vence. A data escolhida vale até o fim do dia, no fuso do seu navegador."/>
          </div>
        </FormSection>
    ...
    {tab==='pratica'&&practical.length>0&&<form className="flex flex-col gap-5" onSubmit={async e=>{e.preventDefault();const f=new FormData(e.currentTarget);setBusy(true);const r=await retractPracticalEvidence(str(f,'event'),str(f,'reason'));setBusy(false);setNotice(r.ok?'Correção registrada. A evidência original permanece na auditoria e deixa de contar a partir de agora.':r.error);if(r.ok)router.refresh();}}>
      <FormSection title="Corrigir uma evidência prática" description="A correção não apaga nem edita o registro original. Ela acrescenta um evento de retratação, e a evidência deixa de contar a partir de agora.">
        <SelectField scope="correcao" name="event" label="Registro a desconsiderar" required>{practical.map(e=><option key={e.id} value={e.id}>{students.find(s=>s.id===e.userId)?.full_name??'Aluno'} · {e.label} · {new Date(e.at).toLocaleDateString('pt-BR')}</option>)}</SelectField>
        <Field scope="correcao" name="reason" label="Motivo da correção" required maxLength={240}/>
      </FormSection>
      <FormActions><Button type="submit" variant="secondary">Registrar correção</Button></FormActions>
    </form>}
    {tab==='acessos'&&<form className="flex flex-col gap-5" onSubmit={async e=>{e.preventDefault();const f=new FormData(e.currentTarget);setBusy(true);const r=await revokeKnowledgeAccess(str(f,'student'));setBusy(false);setNotice(r.ok?'A concessão individual foi encerrada. Assinaturas ativas continuam dando acesso.':r.error);}}>
      <FormSection title="Encerrar uma concessão individual" description="Encerrar não apaga o registro: marca a vigência como terminada agora. Quem tiver assinatura ativa continua com acesso por esse outro caminho.">
        <SelectField scope="revogar" name="student" label="Aluno" required className="max-w-md">{students.map(s=><option key={s.id} value={s.id}>{s.full_name??'Aluno sem nome'}</option>)}</SelectField>
      </FormSection>
      <FormActions><Button type="submit" variant="secondary">Encerrar concessão</Button></FormActions>
    </form>}
    </fieldset>
  </div>;
}
