...
export default function WorkshopBuyForm({ eventId, title }: { eventId: string; title: string }) {
  ...
  if (result?.ok && result.mode === "manual") {
    ...
    return (
      <div className="rounded-2xl border border-brand-green/30 bg-brand-green/10 p-6 text-center">
        ...
        <p className="mt-2 text-sm text-slate-300">Fale com a gente pelo WhatsApp pra finalizar. O link do workshop chega por e-mail após o pagamento.</p>
        {digits && <a href={`https://wa.me/${digits}?text=${msg}`} target="_blank" rel="noreferrer" className="mt-5 inline-block rounded-xl bg-gradient-to-r from-brand-green to-brand-blue px-6 py-3 text-sm font-semibold text-ink-900">Falar no WhatsApp</a>}
      </div>
    );
  }
  ...
  return (
    <form onSubmit={onSubmit} className="space-y-4">
      ...
      <input name="email" type="email" required placeholder="Seu e-mail" className={field} />
      <input name="phone" placeholder="WhatsApp com DDD" className={field} />
      <input name="cpf" inputMode="numeric" placeholder="CPF (para o pagamento)" className={field} />
      ...
      <button disabled={loading} className="w-full rounded-xl bg-gradient-to-r from-brand-green to-brand-blue px-6 py-3.5 text-sm font-semibold text-ink-900 transition-transform hover:scale-[1.02] disabled:opacity-60">
        {loading ? "Gerando pagamento..." : "Garantir minha vaga"}
      </button>
      <p className="text-center text-xs text-slate-500">Pagamento único (Pix ou cartão). O link do workshop chega por e-mail após a confirmação.</p>
    </form>
  );
}
