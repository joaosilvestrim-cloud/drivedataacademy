...
export interface Attempt {id:string;name:string;difficulty:Difficulty;createdAt:string;decisions:Decision[]}
...
export function readSave(text:string):Save {
  ...
  const attempts=data.attempts.map((a:Attempt)=>{
    ...
    ids.add(a.id);const decisions=a.decisions.map(validateDecision);replay(a.difficulty,decisions);
    return {id:a.id,name:a.name,difficulty:a.difficulty,createdAt:a.createdAt,decisions};
  });
...
export function newAttempt(difficulty:Difficulty,name:string):Attempt {
  return {id:crypto.randomUUID(),name:name.trim().slice(0,80)||'Minha estratégia',difficulty,createdAt:new Date().toISOString(),decisions:[]};
}
